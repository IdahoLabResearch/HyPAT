...
# Imports for the ORingsAndDefaultVars class
import numpy as np
import os
import platform
...
class InputForm(tk.Frame):
    ...
    def create_secondary_est_frame(self, row, column):
        ...
        settings_b = ttk.Button(parent, text='Settings', command=self.edit_orings_and_default_vars)
    ...
    def edit_orings_and_default_vars(self):
        ...
        x = self.winfo_rootx()
        y = self.winfo_rooty()
        # Call the O-Rings and Default Variables window and, if something was changed, refresh appropriate values
        o_change, d_change = ORingsAndDefaultVars(storage=self.storage, pos=(x, y)).show_pv_window()
        if o_change:
        ...
        if d_change:
            ...
            self.input['cal_leak_r'].insert(0, "{}".format(
                self.storage.defaults_info["Calibrated Leak Rate [mol(D2) s^-1 Torr^-1]"][0]))
            self.storage.cal_leak_rate.set(self.storage.defaults_info["Calibrated Leak Rate [mol(D2) s^-1 Torr^-1]"][0])

            self.input['sV'].delete(0, "end")
            self.input['sV'].insert(0, "{}".format(
                self.storage.defaults_info["Secondary Volume [cc]"][0]))
            self.storage.sV.set(self.storage.defaults_info["Secondary Volume [cc]"][0])
    ...
    def create_primary_input_frame(self, parent, row, column):
        ...
        self.add_entry2(self, box2, variable=self.input, key="pP_T2",
                        text="Primary calculated D\u2082 partial pressure: pP", subscript="T", subsubscript="2",
    ...
    def create_secondary_input_frame(self, parent, row, column):
        ...
        self.add_entry(self, box3, self.input, key="sV", text="Secondary volume: sV", subscript="",
    ...
    def create_pressure_frame(self, parent, row, column):
        ...
        self.add_text(box4, text="Estimated D permeability: Pr", subscript="D ",
                      # units="[mol (D2) m^-1 s^-1 Pa^-0.5]"
                      units=u"[mol(D\u2082) m\u207b\u00b9 s\u207b\u00b9 Pa\u207b\u2070\u1427\u2075]",
                      tvar=self.storage.Pr_D, row=1)
        self.add_text2(box4, text="Estimated  D\u2082  permeation flux: F", subscript="D", subsubscript="2",  # D_2
                       # units=("[mol (D2) m^-2 s^-1]", "[D m^-2 s^-1]")
                       units=(u"[mol(D\u2082) m\u207b\u00b2 s\u207b\u00b9]", u"[D m\u207b\u00b2 s\u207b\u00b9]"),
                       tvar1=self.storage.F_D2, tvar2=self.storage.F_D2_D, row=2)
        self.add_text(box4, text="Estimated  D\u2082 permeation flux \u00d7 thickness: x",  # D_2 ... (times) ...
                      innersubscript="samp", innertext="F", subscript="D", subsubscript="2",
                      units=u"[mol(D\u2082) m\u207b\u00b9 s\u207b\u00b9]",  # "[mol (D2) m^-1 s^-1]",
                      tvar=self.storage.x_sampxF_D2, row=3)
        self.add_text(box4, text="Estimated  D\u2082 permeation rate: Q", subscript="D", subsubscript="2",  # D_2
                      units=u"[mol(D\u2082) s\u207b\u00b9]",  # "[mol (D2) s^-1]",
                      tvar=self.storage.Q_D2, row=4)
        ...
        self.add_text(box4, text="Detectable with capacitance manometer (>1E-4 Torr):", subscript="",
                      units="YES=detectable, NO=undetectable", tvar=self.storage.DET_SS_QMS, row=7)
        self.add_text(box4, text="Estimated mass 4 pressure in primary side QMS: p", subscript="mass 4",
                      units="[Torr]", tvar=self.storage.p_mass_4, row=8)
        self.add_text(box4, text="Detectable with primary side QMS (>1E-10 Torr):", subscript="",
                      units="YES=detectable, NO=undetectable", tvar=self.storage.Det_PS_QMS, row=9)
        self.add_text(box4, text="Saturate with primary side QMS (>1E-6 Torr):", subscript="",
                      units="SATURATE or OK", tvar=self.storage.Sat_PS_QMS, row=10)
    ...
    def create_final_output_frame(self, row, column):
        ...
        self.add_text(parent, text="Estimated lag time: t", subscript="L",
                      units="[sec]", tvar=self.storage.t_L, row=0)
        self.add_text(parent, text="Estimated  D\u2082 permeation rate: Q", subscript="D", subsubscript="2",  # D_2
                      units=u"[mol(D\u2082) s\u207b\u00b9]",  # "[mol (D2) s^-1]",
                      tvar=self.storage.Q_D2, row=1)
        self.add_text(parent, text="Estimated mass 4 pressure in primary side QMS: p", subscript="mass 4",
                      units="[Torr]", tvar=self.storage.p_mass_4, row=2)
...
class ORingsAndDefaultVars(tk.Toplevel):
    ...
    def __init__(self, storage, pos, *args, **kwargs):
        ...
        self.cal_leak_r = tk.DoubleVar(value=self.defaults_info["Calibrated Leak Rate [mol(D2) s^-1 Torr^-1]"][0])
        # secondary volume
    ...
    def create_inputs(self):
        ...
        self.add_entry(self, parent2, variable=self.inputs, key="cal_leak_rate", text="Calibrated Leak Rate:",
                       subscript="", tvar=self.cal_leak_r, units="[mol(D2) s^-1 Torr^-1]",
        ...
        self.add_entry(self, parent2, variable=self.inputs, key="sV", text="Secondary Volume:",
                       subscript="", tvar=self.sV, units="[cc]", row=3, column=3, in_window=True,
                       command=lambda tvar, variable, key, pf: self.storage.check_for_number(tvar, variable, key,
                                                                                             parent_frame=pf))

        submit_defaults_button = ttk.Button(parent2, text="Submit Default Variables", command=self.submit_4default)
    ...
    def submit_4default(self):
        ...
        if sum(map(np.isnan, [self.cal_leak_r.get(), self.sV.get()])) > 0:
            ...
            if ans:
                ...
                self.storage.defaults_info.loc[0, "Calibrated Leak Rate [mol(D2) s^-1 Torr^-1]"] = self.cal_leak_r.get()
                self.storage.defaults_info.loc[0, "Secondary Volume [cc]"] = self.sV.get()

                # overwrite the old source file with the updated dataframe
                default_entry_vars_filename = os.path.join('datafiles', 'default_entry_vars.xlsx')
